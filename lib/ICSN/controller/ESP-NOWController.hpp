...
private:
  ...
  bool buildPacketForAddress(const uint8_t txAddress[6], const ESP_NOWControlData& data,
...
public:
  ...
  bool initializeCommunication(const char* configPath, uint8_t myMac[6], esp_now_recv_cb_t recvCb,
  ...
  bool processReceivedPacket(const uint8_t myMac[6], const uint8_t senderMac[6],
                             const uint8_t* data, int len, ReceiveProcessResult* result = nullptr);
